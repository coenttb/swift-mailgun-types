...
struct LogsRouterTests {
    ...
    func testAnalyticsURL() throws {
        ...
        let request = Mailgun.Reporting.Logs.Analytics.Request(
            action: "delivered",
            groupBy: "domain",
            startDate: Date(timeIntervalSince1970: 1640995200), // 2022-01-01
            endDate: Date(timeIntervalSince1970: 1641081600), // 2022-01-02
            filter: .init(
                and: [
        ...
        if case let .string(value) = match.analytics?.filter?.and?.first?.value {
